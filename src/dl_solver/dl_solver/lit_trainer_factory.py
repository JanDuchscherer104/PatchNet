...
class CachePlotSampleCallback(Callback):
    def __init__(self):
        super().__init__()
    ...
    def on_validation_batch_end(
        ...
        # Randomly cache one sample from one of the validation batches
        if (
            self.cached_sample is None and np.random.rand() < 0.1
        ):  # Example probability to cache
            # Cache the sample (you might want to deep copy to ensure no overwrites)
            self.cached_sample = batch

    def on_validation_epoch_end(self, trainer, pl_module):
        if self.cached_sample is not None:
            # Get the dataset from the trainer's datamodule
            jigsaw_dataset = trainer.datamodule.jigsaw_val

            # Decompose the cached sample
            X, y = self.cached_sample
            pos_seq = pl_module.model(
                X
            )  # You may need to adjust this line based on your model

            # Ensure the data is moved to CPU for plotting if necessary
            pieces_and_labels = (X.cpu(), pos_seq[0].cpu())

            # Plot the sample
            figure = jigsaw_dataset.plot_sample(
                pieces_and_labels=pieces_and_labels, is_plot=False
            )

            # Optionally, clear the cache after plotting
            self.cached_sample = None
